...
def validate_release_examples() -> list[str]:
    errors: list[str] = []
    examples = {
        ROOT / "README.md": 'Revision = new RevisionMetadata { Author = "OfficeAgent Quickstart" }',
        ROOT / "docs" / "getting-started.md": 'Revision = new RevisionMetadata { Author = "OfficeAgent Quickstart" }',
        ROOT / "docs" / "choose-officeagent.md": 'Revision = new RevisionMetadata { Author = "OfficeAgent Selection Trial" }',
        ROOT / "samples" / "QuickEdit" / "Program.cs": 'Revision = new RevisionMetadata { Author = "QuickEdit" }',
    }
    for path, required in examples.items():
        text = path.read_text(encoding="utf-8")
        if required not in text:
            errors.append(f"tracked-edit example omits explicit revision identity: {path.relative_to(ROOT)}")

    for path in (ROOT / "docs" / "getting-started.md", ROOT / "docs" / "choose-officeagent.md"):
        if "dotnet new console --framework net8.0" in path.read_text(encoding="utf-8"):
            errors.append(f"quickstart requires an SDK-specific template pack: {path.relative_to(ROOT)}")

    api_reference = ROOT / "docs" / "csharp-api.md"
    marker = "<!-- Generated by tools/OfficeAgent.ApiDocs. Do not edit by hand. -->"
    if not api_reference.is_file() or not api_reference.read_text(encoding="utf-8").startswith(marker):
        errors.append("generated C# API reference is missing or lacks its generator marker")
    ...
    for workflow in sorted((ROOT / ".github" / "workflows").glob("*.yml")):
        for reference in uses_pattern.findall(workflow.read_text(encoding="utf-8")):
            if not re.fullmatch(r"[0-9a-f]{40}", reference):
                errors.append(
                    f"workflow action is not pinned to a commit: {workflow.relative_to(ROOT)}@{reference}"
                )
    return errors
